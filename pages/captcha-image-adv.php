...
$im = @ImageCreate (300, 100) // Width and hieght of the image. 
or die ("Cannot Initialize new GD image stream");
$background_color = ImageColorAllocate ($im, 100, 100, 160); // Assign background color
...
$background_color = ImageColorAllocate ($src, 100, 100, 160); // Assign background color

$string= str_shuffle($string);
$text = substr($string,30,1); // One char of the random chars
ImageString($src,50,10,0,$text,$text_color); // 
/*if($i<3){$angle=rand(10,60);}
else{$angle=0;}
*/
$angle=rand(60,80);
$src = imagerotate($src, $angle, 0.5);
$x=$i*50;
imagecopy($im, $src, $x, 40, 0, 0, 20, 20);
//ImagePng ($src);
